...
void Frame_Test(void)
{
		...
		uint16_t FPS,Total_Frame;
		T_Start = HAL_GetTick();
		//for(uint16_t i=0;i<512;i++)//阻塞式帧率测试
		#ifndef IIC_Mode_Blocking
		{
			#warning "IIC_Mode_Blocking is NOT DEFINED"
			Total_Frame=4000;
			while(frame != 4000)//非阻塞式帧率测试
			{
					OLED_Reverse();
					OLED_Update();
			}
		...
		#else
		{
			#warning "IIC_Mode_Blocking is DEFINED"
			Total_Frame=512;
			for(uint16_t i=0;i<512;i++)//阻塞式帧率测试
			{
					OLED_Reverse();
		...
		T_End = HAL_GetTick();
		HAL_Delay(100);
		FPS = Total_Frame/((T_End-T_Start)/1000.0000000);
