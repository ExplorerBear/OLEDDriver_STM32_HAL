...
/** 函数：OLED_Transmit_Datas
	* 说明：此函数只在中断模式或DMA模式下被OLED_Update调用，同时在一组数据传输完成后
	*  			在传输完成回调函数中被调用以传输下一组数据，直到刷新整页完成后，通过读取
	*				OLED_state[bit:7],PageNum[bit:7]来确定是否再一轮的刷新
	*				当IIC运行在阻塞模式下，该函数不会被调用
	*/
void OLED_Transmit_Datas(void)
{
...
void OLED_Update(void)
{
	...
	OLED_state |= (0x1<<7);//设置刷新状态标志位
	OLED_Width = 128;
	
	PageNum = 0x07;
	OLED_state = (OLED_state&0x80)| 0x00;
	OLED_Transmit_Datas();
	#endif
	#ifdef IIC_Mode_Blocking
	uint8_t j;
	...
	for (j = 0; j < 8; j ++)
	{
		...
		OLED_WriteData((OLED_DisplayBuf+(j*128)), 128);
	}
	#endif
...
void OLED_UpdateArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	#ifndef IIC_Mode_Blocking
	if(127-X > Width) W_Data=Width; 
	else W_Data = 127-X;
	if(Page1 >7) PgEn=7;
	else PgEn=Page1;//判断是否超出屏幕，超出不显示
	
	PageNum=(Page<<4)|PgEn;
	OLED_state = (OLED_state&0x80)| X;
	OLED_Width = W_Data;
	...
	for (j = Page; j < Page1; j ++)
	{
		if (X >= 0 && X <= 127 && j >= 0 && j <= 7)		//超出屏幕的内容不显示
		{
			...
			OLED_WriteData((OLED_DisplayBuf+(j*128)+X), Width);
		}
	} 
	#endif
...
void OLED_Clear(void)
{
	...
	for (j = 0; j < 8; j ++)				//遍历8页
	{
		...
		{
			*(OLED_DisplayBuf+(j*128)+i) = 0x00;	//将显存数组数据全部清零
		}
	}
}
...
void OLED_Test(void)
{
	...
	for (j = 0; j < 8; j ++)				//遍历8页
	{
		for (i = 0; i < 128; i ++)			//遍历128列
		{
			*(OLED_DisplayBuf+(j*128)+i) = 0xff;	//将显存数组数据全部置1
		}
	}
}
...
void OLED_ClearArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Y; j < Y + Height; j ++)		//遍历指定页
	{
		for (i = X; i < X + Width; i ++)	//遍历指定列
		{
			...
			{
				*(OLED_DisplayBuf+((j / 8)*128)+i) &= ~(0x01 << (j % 8));	//将显存数组指定数据清零
			}
		}
	}
}
...
void OLED_Reverse(void)
{
	...
	for (j = 0; j < 8; j ++)				//遍历8页
	{
		for (i = 0; i < 128; i ++)			//遍历128列
		{
			*(OLED_DisplayBuf+(j*128)+i) ^= 0xFF;	//将显存数组数据全部取反
		}
	}
}
...
void OLED_ReverseArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height)
{
	...
	for (j = Y; j < Y + Height; j ++)		//遍历指定页
	{
		for (i = X; i < X + Width; i ++)	//遍历指定列
		{
			if (i >= 0 && i <= 127 && j >=0 && j <= 63)			//超出屏幕的内容不显示
			{
				*(OLED_DisplayBuf+((j / 8)+128)+i) ^= 0x01 << (j % 8);	//将显存数组指定数据取反
			}
		}
	}
}
	...
	for (j = 0; j < (Height - 1) / 8 + 1; j ++)
	{
		...
		for (i = 0; i < Width; i ++)
		{
			if (X + i >= 0 && X + i <= 127)		//超出屏幕的内容不显示
			{
				...
				{
					/*显示图像在当前页的内容*/
					*(OLED_DisplayBuf+((Page + j)*128)+(X + i)) |= Image[j * Width + i] << (Shift);
				}
				
				if (Page + j + 1 >= 0 && Page + j + 1 <= 7)		//超出屏幕的内容不显示
				{					
					/*显示图像在下一页的内容*/
					*(OLED_DisplayBuf+((Page + j + 1)*128)+(X + i)) |= Image[j * Width + i] >> (8 - Shift);
				}
			}
		}
	}
...
void OLED_DrawPoint(int16_t X, int16_t Y)
{
	...
	{
		/*将显存数组指定位置的一个Bit数据置1*/
		*(OLED_DisplayBuf+((Y / 8)*128)+X) |= 0x01 << (Y % 8);
	}
}
...
uint8_t OLED_GetPoint(int16_t X, int16_t Y)
{
	...
	{
		/*判断指定位置的数据*/
		if (*(OLED_DisplayBuf+((Y / 8)*128)+X) & 0x01 << (Y % 8))
		{
			return 1;	//为1，返回1
		}
	}
